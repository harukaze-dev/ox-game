...
function showToast(message) { toastPopup.textContent = message; toastPopup.classList.remove('hidden'); setTimeout(() => toastPopup.classList.add('hidden'), 2500); }
function updateProfileSetupUI() {
    ...
    fanMultiStreamerOptions.classList.add('hidden'); fanGroupDisplay.classList.add('hidden');
    if (role === 'streamer') { const s = gameConfig.streamers.find(s => s.id === streamerSelect.value); if (s) nicknameInput.value = s.name; } 
...
function renderGame(state) {
    ...
    const activeFans = players.filter(p => p.role === 'fan' && p.status === 'active'), eliminatedFans = players.filter(p => p.role === 'fan' && p.status === 'eliminated');
    ...
    if (state.gameState === 'lobby') renderLobby(isStreamer);
    else if (state.gameState === 'question') renderQuestion(state.currentQuestion, isStreamer);
    else if (state.gameState === 'finished') renderGameOver(players.find(p => p.role === 'fan' && p.status === 'active'));
}
...
function renderQuestion(question, isStreamer) {
    ...
    question.choices.forEach((text, i) => {
        ...
        if (isMyTurn && !isStreamer) box.onclick = () => socket.emit('submit answer', i); else box.classList.add('disabled');
...
function renderPlayers(players, isStreamer) {
    ...
    Object.values(players).forEach(p => {
        ...
        if (isStreamer) {
            ...
            token.onclick = () => { playerToKick = p; kickPlayerName.textContent = p.nickname; kickConfirmModal.classList.remove('hidden'); };
        }
...
function handleRoundResult(result) {
    ...
    Array.from(choicesContainer.children).forEach((box, i) => { box.onclick = null; box.classList.remove('disabled'); if (i == result.correctAnswer) box.classList.add('correct'); else box.classList.add('incorrect'); });
    document.querySelectorAll('.player-token').forEach(t => { const p = result.players[t.id.split('-')[1]]; if (p) { if(p.answer == result.correctAnswer) t.classList.add('correct-answer'); else if(p.answer !== null) t.classList.add('incorrect-answer');}});
...
function openQuestionerSelectModal(isForRevival) {
    ...
    Object.values(currentGameState.players).filter(p => p.role === 'streamer').forEach(streamer => {
        ...
        item.onclick = () => { socket.emit('designate questioner', { questionerId: streamer.id, isRevival: isForRevival }); selectQuestionerModal.classList.add('hidden'); };
...
function initialize() { 
    ...
    gameConfig.streamers.forEach(s => { const o = document.createElement('option'); o.value = s.id; o.textContent = s.name; streamerSelect.appendChild(o); }); 
...
createRoomBtn.addEventListener('click', () => { userIntent = 'create'; mainMenu.classList.add('hidden'); profileSetup.classList.remove('hidden'); profileSetupTitle.textContent = '방 만들기 (스트리머)'; profileSetupDescription.textContent = '스트리머로 방을 생성합니다.'; document.getElementById('role-streamer').checked = true; updateProfileSetupUI(); });
joinRoomBtn.addEventListener('click', () => { const code = roomCodeInput.value.trim().toUpperCase(); if (!code) return showToast('초대 코드를 입력해주세요.'); userIntent = 'join'; roomToJoin = code; socket.emit('get room info', code); });
...
confirmProfileBtn.addEventListener('click', () => {
    const role = document.querySelector('input[name="role"]:checked').value, nickname = nicknameInput.value.trim();
    if (role === 'fan' && !nickname) return showToast('닉네임을 입력해주세요!');
    const userData = { nickname, role };
    if (role === 'streamer') { const s = gameConfig.streamers.find(s => s.id === streamerSelect.value); userData.nickname = s.name; userData.pfp = s.pfp; userData.streamerId = s.id; userData.streamerKey = streamerKeyInput.value.trim(); }
    else { const fanGroup = fanGroupSelect.value; if (fanMultiStreamerOptions.classList.contains('hidden')) { userData.fanGroup = fanGroupDisplay.dataset.fandomId; } else { if (!fanGroup) return showToast('응원할 스트리머를 선택해주세요.'); userData.fanGroup = fanGroup; } }
    if (userIntent === 'create') socket.emit('create room', userData); else if (userIntent === 'join') socket.emit('join room', { roomId: roomToJoin, userData });
});
...
endRoundBtn.onclick = () => { if (timerAnimationId) cancelAnimationFrame(timerAnimationId); socket.emit('end round'); };
...
questionForm.addEventListener('submit', e => {
    e.preventDefault();
    const text = questionText.value.trim(), choices = Array.from(choicesInputArea.querySelectorAll('input[type="text"]')).map(i => i.value.trim()), radio = choicesInputArea.querySelector('input[name="correctAnswer"]:checked'), duration = parseInt(questionTimerInput.value, 10) || 20;
...
confirmKickBtn.onclick = () => { if (playerToKick) socket.emit('kick player', playerToKick.id); kickConfirmModal.classList.add('hidden'); };
cancelKickBtn.onclick = () => kickConfirmModal.classList.add('hidden');
gameOverCloseBtn.onclick = () => { gameOverModal.classList.add('hidden'); if(currentUserData.role === 'streamer') showToast('게임을 다시 시작하려면 "게임 초기화" 또는 "문제 출제" 버튼을 누르세요.'); };
...
socket.on('room info response', ({ streamersInRoom }) => {
    mainMenu.classList.add('hidden'); profileSetup.classList.remove('hidden'); profileSetupTitle.textContent = '방 참여하기';
    document.getElementById('role-fan').checked = true; updateProfileSetupUI();
    if (streamersInRoom.length === 1) {
        const s = streamersInRoom[0], fandom = gameConfig.streamers.find(c => c.id === s.streamerId).fandom;
        fanGroupDisplay.classList.remove('hidden'); fanGroupDisplay.textContent = `${fandom.name} 팬덤으로 자동 참여됩니다.`;
        fanGroupDisplay.dataset.fandomId = fandom.id; fanMultiStreamerOptions.classList.add('hidden');
    } else {
        fanGroupDisplay.classList.add('hidden'); fanMultiStreamerOptions.classList.remove('hidden');
        ...
        streamersInRoom.forEach(s => {
            ...
            const opt = document.createElement('option'); opt.value = fandom.id; opt.textContent = fandom.name;
...
socket.on('your turn to question', ({ isRevival }) => {
    ...
    questionForm.reset(); updateChoiceInputs();
    createQuestionModal.classList.remove('hidden');
});
socket.on('game message', (text) => { showToast(text); });
socket.on('game state update', (state) => {
    if (!currentUserData.id) currentUserData = state.players[socket.id] || {};
    currentRoomId = state.roomId;
    profileSetup.classList.add('hidden'); mainMenu.classList.add('hidden'); gameContainer.classList.remove('hidden');
    renderGame(state);
});
socket.on('player moved', ({ playerId, answerIndex }) => { const t = document.getElementById(`player-${playerId}`), c = choicesContainer.children[answerIndex]; if (t && c) c.appendChild(t); });
socket.on('round result', handleRoundResult);
socket.on('error message', (message) => { alert(message); window.location.reload(); });