...
function mapToObject(map) { const obj = {}; for (let [k, v] of map.entries()) { obj[k] = v instanceof Map ? mapToObject(v) : v; } return obj; }
...
function broadcastGameState(roomId) {
    ...
    const gameState = {
        roomId, ownerId: room.ownerId, players: mapToObject(room.players),
        gameState: room.gameState, currentQuestion: room.currentQuestion ? questionForClient : null,
...
io.on('connection', (socket) => {
    ...
    socket.on('create room', (userData) => {
        if (userData.role !== 'streamer') return socket.emit('error message', '방은 스트리머만 만들 수 있습니다.');
        if (userData.streamerKey !== 'mhyam3') return socket.emit('error message', '스트리머 인증 키가 올바르지 않습니다.');
        ...
        rooms.set(roomId, {
            ownerId: socket.id, players: new Map([[socket.id, { ...socket.userData, status: 'active', answer: null }]]),
            gameState: 'lobby', currentQuestion: null, isRevival: false,
            roundTimer: null, currentQuestionerId: null
        });
    ...
    socket.on('join room', (data) => {
        ...
        const room = rooms.get(roomId);
        if (!room) return socket.emit('error message', '존재하지 않는 방입니다.');
        if (userData.role === 'streamer' && userData.streamerKey !== 'mhyam3') return socket.emit('error message', '스트리머 인증 키가 올바르지 않습니다.');
        if (userData.role === 'fan') {
            if (!userData.fanGroup) return socket.emit('error message', '팬덤 정보가 없습니다.');
            const streamerConfig = config.streamers.find(s => s.fandom.id === userData.fanGroup);
            if (!streamerConfig) return socket.emit('error message', '선택한 팬덤 정보를 찾을 수 없습니다.');
            
            // [수정] 팬덤 pfp와 함께 스트리머의 고유 색상 정보도 userData에 추가
    ...
    socket.on('submit answer', (answerIndex) => {
        ...
        if (canSubmit) { player.answer = answerIndex; io.to(socket.roomId).emit('player moved', { playerId: socket.id, answerIndex }); }
    });
    ...
    const endRoundLogic = (roomId) => {
        ...
        let activeFans = [];
        if (room.isRevival) {
            room.players.forEach(p => { if (p.status === 'eliminated' && p.answer == correctAnswer) p.status = 'active'; });
            activeFans = Array.from(room.players.values()).filter(p => p.role === 'fan' && p.status === 'active');
        } else {
            room.players.forEach(p => { if (p.role === 'fan' && p.status === 'active' && p.answer != correctAnswer) p.status = 'eliminated'; });
            activeFans = Array.from(room.players.values()).filter(p => p.role === 'fan' && p.status === 'active');
        }
        if (!room.isRevival && activeFans.length <= 1) room.gameState = 'finished';
        ...
        room.isRevival = false; room.currentQuestionerId = null; 
    };
    ...
    socket.on('reset game', () => {
        ...
        if(room.roundTimer) clearTimeout(room.roundTimer);
        room.gameState = 'lobby'; room.currentQuestion = null; room.isRevival = false; room.roundTimer = null; room.currentQuestionerId = null;
        room.players.forEach(p => { p.status = 'active'; p.answer = null; });
    ...
    socket.on('kick player', (playerId) => {
        ...
        if (targetSocket) { targetSocket.emit('error message', '방에서 강퇴당했습니다.'); targetSocket.leave(socket.roomId); }
